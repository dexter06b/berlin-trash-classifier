...
- **Relationships**:
  - Referenced by many `DisposalItem` objects as the primary recommended outcome
...
- **Relationships**:
  - References one `DisposalOutcome` as the primary answer
  - Contains many `RuleNote` entries
...
- **Relationships**:
  - Nested under one `DisposalItem`

## QuizQuestion
- **Purpose**: Runtime representation of a prompt shown during a quiz round.
- **Fields**:
  - `item_slug`: linked disposal item identifier
...
- **Relationships**:
  - Derived client-side from `DisposalItem` and `DisposalOutcome`
...
- **Fields**:
  ...
  - `recent_item_slugs`
- **Relationships**:
  - Stored in browser-side persistence rather than a user account

## Validation Rules
- Every active `DisposalItem` must reference one primary `DisposalOutcome`.
